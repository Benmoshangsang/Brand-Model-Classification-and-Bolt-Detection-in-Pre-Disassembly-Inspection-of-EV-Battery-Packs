    ...
    dict(
        ...
        transforms=[
            [dict(
                ...
                scales=[(480, 900), (512, 900), (544, 900), (576, 900), (608, 900), (640, 900)],
                keep_ratio=True)]
        ]),
    dict(
        type='PackDetInputs',
...
model = dict(
    ...
    backbone=dict(
        _delete_=True,
        type='TransformerBackbone',  # 你的自定义骨干（注册在 MODELS）
        out_indices=(0, 1, 2, 3),
        pretrained=None,
        depths=(1, 3, 8, 4),
        window_size=(8, 8, 8, 8),
    ...
    neck=dict(
        type='FPN',
        in_channels=[80, 80, 80, 80],  # 与 TransformerBackbone 的每级通道数一致
        # 若后续改为逐级升维(如 80/160/320/640)，同步改成 [80,160,320,640]
        out_channels=256,
        num_outs=4),
    rpn_head=dict(
        type='RPNHead',
    ...
    roi_head=dict(
        ...
        # ✅ 修正 reg_class_agnostic 且用 deepcopy 生成 3 份
        bbox_head=[deepcopy(dict(
            type='ConvFCBBoxHead',
            num_shared_convs=4,
            num_shared_fcs=1,
            in_channels=256,
            ...
            reg_class_agnostic=False,   # ← 这里修正了键名
...
work_dir = './work_dirs/cascade_mask_rcnn_transformer_coco_bolts'
