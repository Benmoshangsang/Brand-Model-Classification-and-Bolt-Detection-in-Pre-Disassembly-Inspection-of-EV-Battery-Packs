...
class CSDSLayer(nn.Module):
    ...
    def __init__(
        ...
        if prev_dim is not None:
            ...
            self.sl_proj_in = None # 第一个 stage 没有输入状态
    ...
    def forward(self, x: torch.Tensor, sl_bridge_state: Optional[torch.Tensor] = None):
        ...
        for blk in self.blocks:
            # 如果启用梯度检查点，则使用 checkpoint 包装 block 的前向传播
            if self.training and self.use_checkpoint:
                tokens, m_state_blk = checkpoint.checkpoint(blk, tokens, sl_bridge_state=sl, use_reentrant=False) # ✅ PyTorch 2.x 推荐 use_reentrant=False
            else:
        ...
        # ✅ 【数值稳定性增强】增加 nan_to_num 作为安全保障，防止 inf/nan 在前向传播中扩散
...
class MambaVision(nn.Module):
    ...
    def __init__(self,
        ...
        cur = 0
        prev_dim = None # ✅ 用于追踪上一层的维度
        for i in range(len(depths)):
            current_dim = int(dim * 2 ** i) # ✅ 当前层的维度
            level = CSDSLayer(
                dim=current_dim,
                depth=depths[i],
                ...
                prev_dim=prev_dim, # ✅ 将上一层的维度传递给当前层
            )
            cur += depths[i]
            prev_dim = current_dim # ✅ 更新 prev_dim 供下一轮循环使用
