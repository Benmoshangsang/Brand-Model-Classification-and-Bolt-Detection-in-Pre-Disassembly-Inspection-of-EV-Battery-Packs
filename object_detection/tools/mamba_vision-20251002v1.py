...
class MambaVisionMixer(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        ln = nn.LayerNorm(self.dt_rank + self.d_state * 2, device=x.device, dtype=x.dtype)
        x_dbl = ln(x_dbl)
...
class PCSScanMamba(nn.Module):
    ...
    def forward(self, tokens_window: torch.Tensor):
        ...
        # 四路共享 Mamba
        yh = self.mamba_core(rh)  # (B,N,C)
        yv = self.mamba_core(rv)
        yd = self.mamba_core(rd)
        ya = self.mamba_core(ra)
...
class CSDSBlock(nn.Module):
    ...
    def __init__(
        ...
        # SAC 桥
        self.sac = SACBridge(dim, num_heads)
    ...
    def forward(self, x_tokens: torch.Tensor, sl_bridge_state: Optional[torch.Tensor] = None):
        ...
        if sl_bridge_state is not None:
            # 简单仿射调制：x = x + alpha*state
            alpha = 0.1
        ...
        # SAC：用 M 状态生成 T 的 q_bias，并将 T 的聚合门控写回 M
        # 先以无偏置注意一次，或者直接以 M 状态生成偏置再注意
...
# -------------------------------------------------------
# Stage（层级）——窗口分割、多个 CSDSBlock、降采样、SL-Bridge 聚合/传递
# -------------------------------------------------------
...
class CSDSLayer(nn.Module):
    ...
    def __init__(
        ...
        # NOTE: 懒初始化的跨 stage 状态投影层，避免 in_features 维度不匹配
        self.sl_proj_in: Optional[nn.Linear] = None
    ...
    def forward(self, x: torch.Tensor, sl_bridge_state: Optional[torch.Tensor] = None):
        ...
        # 若有跨 stage 状态，先做线性调制后传给 block（懒构建保证 in/out 对齐）
        if sl_bridge_state is not None:
            in_c = sl_bridge_state.shape[-1]  # 上一 stage 的通道数
            if (self.sl_proj_in is None) or \
               (self.sl_proj_in.in_features != in_c) or \
               (self.sl_proj_in.out_features != self.dim):
                self.sl_proj_in = nn.Linear(in_c, self.dim).to(sl_bridge_state.device, dtype=sl_bridge_state.dtype)
            sl = self.sl_proj_in(sl_bridge_state)  # (B, self.dim)
