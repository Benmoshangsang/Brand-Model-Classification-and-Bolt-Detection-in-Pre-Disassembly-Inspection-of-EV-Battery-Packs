...
class CustomBrandEvaluator(BaseMetric):
    ...
    def __init__(self,
        ...
        # Ensure both top-1 and top-5 are included
    ...
    def process(self,
        ...
        # Accumulate to self.results (BaseMetric handles aggregation in distributed mode)
        self.results.extend(results)

    def compute_metrics(self, results: List[Tuple[Tensor, Tensor]]) -> Dict[str, float]:
        # Fallback for zero valid samples
        if len(results) == 0:
            if self.debug:
        ...
        # Number of classes: Prefer prediction dimension C; fallback to max(label)+1
        C = int(preds.shape[1]) if preds.ndim == 2 and preds.shape[1] > 0 else int(max(y_true.max(), y_pred.max()) + 1)

        # Confusion Matrix cm[t, p]
        ...
        eps = 1e-12
        # Calculate macro average only on classes present in the ground truth (support > 0)
        # to prevent all-zero classes from skewing results downward.
        support = cm.sum(axis=1)  # Ground truth samples per class
        mask = support > 0
        ...
        if mask.any():
            ...
            # Extreme fallback: No brand labels found in the validation set
    ...
    def accuracy(output: Tensor, target: Tensor, topk=(1,)) -> List[float]:
        ...
        res = []
        for k in topk:
            # Count hits within the top-k rows
